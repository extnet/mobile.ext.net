...
Ext.define('KitchenSink.controller.phone.Main', {
    ...
    onNavTap: function (nestedList, list, index, target, record, e) {
        if (record.isLeaf()) {
            Ext.util.History.on("change", function () {
                // An ugly workaround not to showing (.showView() call) the same view twice if a leaf node is tapped
                // The bug is presented in Sencha Examples Explorer as well

                return false;
            }, null, { single: true, priority: 1 });
    ...
    showView: function (item, direct) {
        ...
        this.createView(item, function (view, goToLeaf) {
            var nav = this.getNav(),
                title = item.get('text'),
     ...
     * @param {KitchenSink.model.Demo} item The Demo model instance for which we want to show a view
     */
    // That is the original ExtJS showView
    //showView: function(item, direct) {
    //    if (this.isProfile(item)) {
    //        return;
    //    }
